...
bool compare_pairs (const pair<int,RideOffer*>& first, const pair<int,RideOffer*>& second)
{
...
void App::findAndPrintRoadMatches(string road){
	int matches = 0;
	list<pair<int,RideOffer*> > matched;
	for(size_t i = 0; i < offers.size(); i++){
		RideOffer* offer = dynamic_cast<RideOffer*>(offers[i]);
		int dist = offer->findDestinationInPath(road);
		if(dist != -1){
			matched.push_front(make_pair(dist,offer));
			matches ++;
		}
	}
	if(matches == 0)
		...
		matched.sort(compare_pairs);
		list<pair<int,RideOffer*> >::iterator it = matched.begin();

		while(it != matched.end()){
			RideOffer* offer = (*it).second;
			cout << (*it).first+1 << "º " << *offer << endl;
			it++;
		}
	}
}
...
void App::findAndPrintUserMatches(string name){
	...
	for(size_t i = 0; i < offers.size(); i++){
		RideOffer* offer = dynamic_cast<RideOffer*>(offers[i]);
		int dist = offer->findPassenger(name);
		if(dist != -1){
			matched.push_front(make_pair(dist,offer));
			matches ++;
		}
